...
{XXX: need a picture showing the architecture. The picture shows that Giza uses existing 
cloud services.}
This paragraph discusses the overall approach. On the data-path, object data is
coded into several fragments, named by their content hashes, and stored in the
blob storage of different DCs.   On the meta-data path, \name stores the set of
content hashes of coded fragments as meta-data of the object.
...
~\Cref{fig:xxxx} shows the architecture of \name.  To support mutable coded
...
We take the approach to layer giza on top of existing cloud storage infrastructure, to take 
advantage of deployed systems.  These existing cloud storage systems have
redundancy within a single data center, but are not geo-replicated.  Thus,
\name must explicitly provide cross data center redundancy through coding and
replication.  On the data path, {\name} stores coded fragments in cloud blob
stores in different data centers, such as Azure's blob store, or Amazon's S3.
On the meta-data path, \name stores version information in existing cloud
tables and replicates them across several data centers.  The number of coded
fragments, and thus the set of data centers storing object data, is
configurable depending on the user desired tradeoff on durability vs. cost.
The number of data center to replicate the meta-data is fixed at 3.
...
{XXX: first challenge is how to build a strongly consistent, geo-replicated meta-data storage
on existing single-DC cloud tables.  Argue that no existing service satisfies our needs.
}

{XXX: second challenge is to jointly optimize the data and meta-data path to achieve a single 

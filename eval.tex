...
The table latency is the latency for a Giza proxy to conditionally update its local DC table. Since Classic Paxos requires two rounds and hence two table updates, its table latency is around twice that of Fast Paxos.
...
However, CockroachDB is currently not optimized to store large objects. We include our experimentation with CockroachDB with 128KB objects for completeness. Each data block for a 128KB object is 64KB, the maximum recommended object size to store in CockroachDB. Reducing the object size furthermore does significantly improve CockroachDB's performance. However, erasure coding small object is not a valid use case for Giza.
